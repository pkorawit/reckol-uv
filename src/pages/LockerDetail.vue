...
<div class="detail">
    ...
    <div class="action">
        <div v-if="!locker.fromOther" class="my-locker flex justify-around">
            <q-btn color="primary" label="unlock" outline :to="`/scanner?mode=${mode.SELF_UNLOCK}`" class="my-locker-btn" />
            <q-btn color="primary" label="sent otp" outline @click="openSendOneTimeCodeDialog" class="my-locker-btn" />
        </div>
        <div v-if="locker.fromOther" class="from-other">
            <q-btn color="primary" label="unlock" outline :to="`/scanner?mode=${mode.OTP_UNLOCK}`" />
        </div>
    </div>
</div>
</template>
...
import {
    getLockerState
} from "src/api/collections/locker-collection";
import {
    MODE
